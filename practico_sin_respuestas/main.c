...
void ppprint(nodo* arbol){
    if(arbol->down != NULL) printf("la wea ta repetida");
  
    if(arbol->left != NULL) ppprint(arbol->left);
    printf("(%i)",arbol->nota);
    if(arbol->right != NULL) ppprint(arbol->right);
}
...
int main(int argc, char** argv) {
  ...
  p = frecuencia_de_una_nota(raiz, 28);
  pauta = 24;
  puntaje = p == pauta ? 0.48 : 0;
  printf("%5d\t\t%5d\t\t%6d\t\t%5.2f\n", 3, p, pauta, puntaje);
  total += puntaje;
